...
async def get_token_service(redis_service: RedisService = Depends(get_redis_service), role_repository: SQLAlchemyRoleRepository = Depends(get_role_repository), user_repository: SQLAlchemyUserRepository = Depends(get_user_repository)):
    """Dependency for token service"""
    return JWTTokenService(redis_service=redis_service, role_repository=role_repository, user_repository=user_repository)
