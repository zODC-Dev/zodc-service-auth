...
from src.domain.events.project_events import JiraUsersFoundEvent
...
class NATSSubscribeService:
    ...
    async def start_nats_subscribers(self) -> None:
        ...
        for event_type in NATSSubscribeTopic:
            if event_type == NATSSubscribeTopic.JIRA_USERS_FOUND:
                await self.nats_service.subscribe(
                    subject=event_type.value,
                    callback=self.handle_jira_users_found_event
                )

    async def handle_jira_users_found_event(self, subject: str, data: Dict[str, Any]) -> None:
        """Route Jira users found event to project service"""
        try:
            event = JiraUsersFoundEvent.model_validate(data)
            await self.project_service.handle_jira_users_found(event)
